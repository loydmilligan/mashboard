...
export function PlaylistSyncPanel({ theme, className }: PlaylistSyncPanelProps): JSX.Element {
  ...
  const handleSync = async (): Promise<void> => {
    ...
    try {
      ...
      const description = `Music League ${selectedTier} for "${theme.title}". Synced via Mashboard.`
...
export function QuickSyncButton({ theme }: { theme: MusicLeagueTheme }): JSX.Element {
  const { setThemePlaylist } = useMusicLeagueStore()
  const [isSyncing, setIsSyncing] = useState(false)
  ...
  const handleQuickSync = async (): Promise<void> => {
    ...
    try {
      ...
      const description = `Music League candidates. Synced via Mashboard.`
