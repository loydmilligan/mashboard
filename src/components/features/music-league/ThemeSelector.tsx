...
function getStatusBadge(theme: MusicLeagueTheme): JSX.Element | null {
...
function getFunnelSummary(theme: MusicLeagueTheme): string {
  const parts: string[] = []
  if (theme.pick) parts.push('1 pick')
  if (theme.finalists.length > 0) parts.push(`${theme.finalists.length} finalists`)
  if (theme.semifinalists.length > 0) parts.push(`${theme.semifinalists.length} semis`)
  if (theme.candidates.length > 0) parts.push(`${theme.candidates.length} candidates`)
  return parts.length > 0 ? parts.join(', ') : 'Empty funnel'
}

export function ThemeSelector({ onNewTheme, className }: ThemeSelectorProps): JSX.Element {
...
export function ThemeSelectorCompact({ className }: { className?: string }): JSX.Element {
