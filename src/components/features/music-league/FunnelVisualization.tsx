...
function TierSection({ config, songs, themeId, compact, onSongClick }: TierSectionProps): JSX.Element {
...
export function FunnelTabs({ theme }: { theme: MusicLeagueTheme }): JSX.Element {
  const [activeTier, setActiveTier] = useState<FunnelTier>('candidates')
  ...
  const activeConfig = TIER_CONFIGS.find((c) => c.tier === activeTier)!
  const activeSongs = getSongsForTier(activeTier)
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex gap-1 p-2 overflow-x-auto">
        {TIER_CONFIGS.map((config) => {
          const count = getSongsForTier(config.tier).length
          return (
            <button
              key={config.tier}
              onClick={() => setActiveTier(config.tier)}
              className={cn(
                'flex items-center gap-1 px-2 py-1 rounded text-xs whitespace-nowrap transition-colors',
                activeTier === config.tier
        ...
        <div className="space-y-2">
          {activeSongs.length === 0 ? (
            <p className="text-xs text-muted-foreground text-center py-4">
              No songs in {activeConfig.label.toLowerCase()}
            </p>
          ) : (
...
export function FunnelSummary({ theme }: { theme: MusicLeagueTheme }): JSX.Element {
  return (
